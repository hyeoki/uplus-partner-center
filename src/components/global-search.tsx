...
export default function GlobalSearch() {
  ...
  // 현재 /search?q= 페이지에서 진입 시 쿼리 동기화
  useEffect(() => {
    const current = params.get("q");
    if (current) setQ(current);
  }, [params]);
  ...
  return (
    <form
      ...
      className="shrink-0 flex items-center gap-2 px-3 h-10 rounded-full overflow-hidden transition-[width,background] duration-300 ease-out focus-within:bg-white w-[140px] focus-within:w-[280px]"
      style={{
